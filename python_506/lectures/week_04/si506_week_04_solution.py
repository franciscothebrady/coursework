...
ev_types = []
for vehicle in ev_vehicles[1:]:
    if vehicle[1] not in ev_types:
        ev_types.append(vehicle[1])
...
# Alternative
# ev_max_range = [None, 0]  # store vals in list
# for vehicle in ev_vehicles[1:]:
#     vehicle_range = int(vehicle[4])  # cast str to int
#     if vehicle_range > ev_max_range[1]:
#         ev_max_range[0] = vehicle[0]  # automaker model
#         ev_max_range[1] = vehicle_range

# print(f"\n2.0.1 Max range ({ev_max_range[1]} mi) = {ev_max_range[0]}")
...
ev_min_range = None
min_range = 10000  # seed value
# min_range = float("inf") # infinity
for vehicle in ev_vehicles[1:]:
    vehicle_range = int(vehicle[4])
    if vehicle_range < min_range:
        ev_min_range = vehicle[0]  # automaker model
        min_range = vehicle_range
...
#         ev_min_range[0] = vehicle[0]  # automaker model
#         ev_min_range[1] = vehicle_range

# print(f"\n2.1 Shortest range ({ev_min_range[1]} mi) = {ev_min_range[0]}")
...
tesla_3_range = ev_vehicles[-1][4]  # Tesla Model 3 AWD
...
for i in range(len(ev_vehicles)):
    if ev_vehicles[i][1].lower() == "suv":
        ev_vehicles[i][1] = "Sports Utility Vehicle"  # rename
...
affordable = []
unaffordable = []
for i in range(1, len(ev_vehicles)):
    msrp = int(ev_vehicles[i][-1][1:].replace(",", ""))
    # msrp = int(ev_vehicles[i][-1].replace("$", "").replace(",", ""))
    vehicle = f"{ev_vehicles[i][0]} {ev_vehicles[i][1]} (msrp = ${msrp:,})"
    if msrp <= 50000:
        affordable.append(vehicle)
    else:
        unaffordable.append(vehicle)
...
# unaffordable = []
# for vehicle in ev_vehicles[1:]:
#     msrp = int(vehicle[-1][1:].replace(",", ""))
#     # msrp = int(vehicle[-1].replace("$", "").replace(",", ""))
#     vehicle = f"{vehicle[0]} {vehicle[1]} (msrp = ${msrp:,})"  # :, adds commas
#     if msrp <= 50000:
#         affordable.append(vehicle)
#     else:
#         unaffordable.append(vehicle)
...
# print(f"\n4.2.1 Affordable EVs for loop (len={len(affordable)}) = {affordable}")
# print(f"\n4.2.1 Unaffordable EVs for loop (len={len(unaffordable)})= {unaffordable}")
