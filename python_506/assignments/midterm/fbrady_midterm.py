...
def clean_squad(squad):
    ...
    # in one line, return as tuple: first part of squad, upper,
    # and the rest of squad, joined to deal with two part country names
    return (squad[:2].upper(), squad[3:])

    # squad = squad.split()
    # squad = tuple([squad.split()[0].upper(), ' '.join(squad.split()[1:])])
    # return tuple([squad[0].upper(), ' '.join(squad[1:])])
    # return squad
    # pass  # TODO Implement
...
def get_multi_position_players(players, pos_idx):
    ...
    for player in players:
        position_list = player[pos_idx].split("|")
...
def get_team_shooting_numbers(team, slice_):
    ...
    for team_member in team:
        goals, shots, shots_on_target = get_player_shooting_numbers(team_member, slice_)
        goals_count += goals
        shot_count += shots
        shots_on_target_count += shots_on_target
    return tuple([goals_count, shot_count, shots_on_target_count])
...
def main():
    ...
    # 1.3
    # iterate over data
    # drop unneeded "columns" required for analysis
    # keep only the first 10 elements of each nested list
    ...
    for player in players:
        ...
        # insert code into the 4th position of the list
        player.insert(squad_idx, code)
        # update the cleaned squad element to be your new squad variable
        player[squad_idx + 1] = squad
    # 3.3
    # update headers
    headers.insert(squad_idx, "Country_Code")
    # 3.4
    # update squad index position
    squad_idx += 1
    # 3.5
