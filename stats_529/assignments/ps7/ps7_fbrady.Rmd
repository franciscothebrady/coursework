...
Reliability has to do with consistency, and whether or not the measure can be replicated by others at a different time. The consulting firm picked 20 spots to conduct their analysis, not in a random way, but based on police activity, and a number of other factors.  
...
Validity has to with how well the scores from a measure represents the variable that it is intended to measure. The consultants set out to measure the difference between a benchmark measure -- how frequently drivers at specific locations were from different racial, ethnic and gender groups, and a stop measure -- how frequently drivers were stopped from each racial, ethnic, or gender group. The measure is intended to score the difference between the benchmark and the number of stops. In this respect it is a valid measure.  
...
##### (c) Now perform a significance test ($\alpha$ = .01) in which the null hypothesis is that there is no difference between the proportion of drivers who are Black and the proportion of traffic stops that involve Black drivers. Perform all the steps and report all relevant statistics.  
  i) assumptions: independent samples, large sample sizes. no assumption of equal variance. proportions so using z tests.  
  ii) hypothesis: $H_0: \hat{\pi_2} - \hat{\pi_1} = 0$, $H_a: \hat{\pi_2} - \hat{\pi_1} \ne 0$  
  iii) Critical value and test statistic:  Critical value at $\alpha_{.01} \approx 2.32$  
  To calculate the test statistic, we first need the standard error using the baseline assumption of no difference -- the proportion of Black drivers stopped would be the same as the population, and to combine the sample, we use both Ns: 
$$
se_0 = \sqrt{\frac{\hat{\pi}(1-\hat{\pi})}{n_1} + \frac{\hat{\pi}(1-\hat{\pi})}{n_2}}
$$
$$
se_0 = \sqrt{\frac{.138(1-.138)}{3042} + \frac{.138(1-.138)}{487}}
$$

```{r}
se_0 <- sqrt(((.138 * .862) / 3042) + ((.138 * .862) / 487) )
se_0
```
...
$$
$$
z = \frac{(.275 - .138) - 0}{0.01683352}
$$
```{r} 
z <- ((.275 - .138) - 0)  / 0.01683352
z
```

  iv) p-value: Our observed test statistic is 8.138524, so we know that our p-value is less than .000000287*2 = 0.000000574
...
<br>  
Yes, the samples are independent, because the life expectancy variable is measured in different samples. Further, the samples are taken from different countries. If the samples were from the same country, before and after a it became a democracy, they would be paired samples, and dependent.  
...
<br>  
We should not make the assumption that life expectancy has the same variance in democracies and non-democracies. One reason is because the standard deviation of democracies is 10.2, and in non-democracies it is 11.3. We would have to run an additional test to accept or reject that the difference between the standard deviations was in fact zero, but at first glance they imply a different variance for each sample. There are many other factors that could contribute to the variance in life expectancy for democracies versus non-democracies, such as political stability, level of development, and in-country wealth distributions.  
...
<br>  
[NOT SURE IF S HERE IS STANDARD DEVIATION OR STANDARD ERROR. SHORTCUT USES STANDARD ERROR. BELOW I ASSUME S MEANS STANDARD DEVIATION AND USE THAT TO CALCULATE THE SE DIFF WITH UNEQUAL VARIANCES USING THE FULL FORMULA]  
...
##### (d) Using the standard error you just calculated, perform a significance test for the difference of means ($\alpha$ = .05).  
  ...
  i) assumptions: not equal variance. Sample sizes are small, so assuming a normal distribution [DOUBLE CHECK THIS]. Means so using a t statistic.  
  ii) hypothesis: $H_0: \mu_2 - \mu_1 = 0$, $H_a: \mu_2 - \mu_1 \neq 0$.  
  iii) Critical value and test statistic:  
  Critical value at $\alpha = .05$, with degrees of freedom $min(n_1, n_2) = 14$: $t_{.025} = 2.145$.  
  To calculate the test statistic, we calculate the difference of means and use $se_{diff}$ calculated above:  
$$
...
##### (f) Perform a significance test for the difference of means under the assumption of equal variances ($\alpha$ = .05).  
  ...
  ii) hypothesis: $H_0: \mu_2 - \mu_1 = 0$, $H_a: \mu_2 - \mu_1 \neq 0$.  
  iii) Critical value and test statistic:  
  We have $n_1 + n_2 - 2 = 90$ degrees of freedom, so the critical value of $t$ (using 80 df from the t-statistic table) for $\alpha = .05$ is: $t_{.025} = 1.990$. To calculate the test statistic:  
$$
...
##### (b) Do this question by hand using the formulas. Using the data from the table, perform a significance test for whether the proportion of people that have health insurance is different across the categories of `BAplus`. Be sure to use the correct formula for the standard error.  
  i) assumptions: [INDEPENDENT?], Random sample of two categorical variables. Large samples, normal distribution, There is an expected frequency of at least 5 cases in each cell:  
$$
E[\pi_{\text{health ins.}}] = \frac{7249}{8058} = 0.8996029
$$
To test -- take the smallest cell and calculate expected proportion: 
$$
202 *.899 = 181.7198
$$
  ii) hypothesis: $H_0$: variables are independent. $H_a$: variables are not independent.  
  iii) Critical value and test statistic:  
$$
\chi^2 = \Sigma\frac{(f_o - f_e)^s}{f_e}
$$
\break  

| | BAPlus | | |
|---- |---- |---- |---- |  
| Health Ins. | Does not have BA | Has BA | Total |  
| Does not have Health Ins. | $\frac{607 - (607*.899)^2}{545.693}$ | $\frac{202 - (202*.899)^2}{181.598}$ | 809 |  
| Has Health Ins. | $\frac{3840 - (3840*.899)^2}{3452.16}$ | $\frac{3409 - (3409*.899)^2}{3064.691}$ | 7249 |  
| |  | | |  
| Total | 4447 | 3611 | 8058 |  

```{r}
# note: this code is for question 4.b:
prop <- 0.8996029
# oe = observed - expected (r, c)
oe_11 <- (607 - (607*0.8996029))^2 / (607*0.8996029)
oe_12 <- (202 - (202*0.8996029))^2 / (202*0.8996029)
oe_12 <- (3840 - (3840*0.8996029))^2 / (3840*0.8996029)
oe_22 <- (3409 - (3409*0.8996029))^2 / (3409*0.8996029)
chi_squared <- (oe_11 + oe_12 + oe_12 + oe_22)
chi_squared
```
  For the $\chi^2$ test of independence, degrees of freedom is:  
$$
df = (r - 1)(c - 1) = (2 - 1)(2 - 1) = 1
$$
  Giving us a critical value at an $\alpha = .05$ of: 3.84. Our observed $\chi^2$ statistic is 131.0475.  
  iv) p-value: [NOT SURE HOW TO COMPUTE P-VALUE HERE]  
  v) conclude: Since our observed statistic is greater than the critical value, we can reject the null hypothesis that having health insurance is independent of having a BA.  
...
In the output, `prop.1` is the proportion of people without a BA who report that they have health insurance. `prop.2` is the proportion who have health insurance with a BA. The $\chi^2$ statistic is 143.18, and the p-value is less than 0.00000000000000022. Thus we can reject the null hypothesis that they are independent. To obtain the z-statistic:  
...
```
  ...
  vi) p-value:  Using the t-table and our degrees of freedom, the closest t value we can find is 3.174. Since our observed t-statistic is 21.7548: $ p < t_{.002}$
  v) conclude:  Since the absolute value of our observed t-statistic exceeds the critical value, we can reject the null hypothesis that the true mean (difference) is zero. Similarly, our p-value is less than $\alpha$ at the .05 confidence level. 
