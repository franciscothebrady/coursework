...
To find the variance of X, we take each x subtracted by the mean and square it, then multiply that value by it's probability. Then we sum all of those values. 

#TODO: write out the math later 
```
...
P(~X) for any A through D = 1 - 0.7 = 0.3 
...
In this case:  
$$
P(1 accept) = \frac{4!}{1! \dot 3!}(0.7)(0.3)^3
$$
```{r}
# 1/4 accepted, in 4 ways
(0.7 * 0.3 * 0.3 * 0.3) + 
  (0.3 * 0.7 * 0.3 * 0.3) + 
  (0.3 * 0.3 * 0.7 * 0.3) + 
  (0.3 * 0.3 * 0.3 * 0.7) 
...
```{r}
# at least 3 accepted, two ways: 3 accepts and 4 accepts 
(0.7 * 0.7 * 0.7 * 0.3) + (0.7 * 0.7 * 0.7 * 0.7)
```
c) P(at least 3) = P(3) + P(4) 
= P(4) (from part a) = .2401 
P(3) = 
$$
\frac{4!}{3!!!}(0.7^3 (0.1)^.1 \\
= 4(0.3)(.343)  \\
= .416 \\
P(\text{ at least 3 }) = P(3) + P(4) \\
= .2401 + .4116  \\
= .6517
$$
...
P(DD or PD or DP) = P(7/12 * 6/11) or P(5/12 * 7/11) or P(7/12 * 5/11)  
```{r}
DD <- (7/12) * (6/11)
PD <- (5/12) * (7/11)  
DP <- (7/12) * (5/11)
...
$$
P(D | P) = \frac{P(PD)}{P(P)} \\
=  \frac{(0.417)(0.636)}{0.417} \\
= 0.636
$$

```{r}
# Todo fill in the math tonight for this 
```
  
(c) Suppose you have not drawn a quarter yet. With what probability will the second quarter be P?  
## for this one just add the probs for both P(PP) + P(DP) 
