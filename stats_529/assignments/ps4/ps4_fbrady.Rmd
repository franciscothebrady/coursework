...
Note for my grader: I made this using Rmarkdown, which can render \LaTeX\. It is useful for equations and including calculations all in one file. Let me know if you have any questions!
...
The z-score for 33 is `r z33`. Looking up that value in the table, and taking the values between 1.62 and 1.63: 0.94738 and 0.94845. Since we need percentages greater than 33 hours, and we are using a positive z table, we take 1 - the values to get our percentages: between 5.155% - 5.262% watch more than 33 hours.  

The z-score for 14 is `r z14`. Looking up that value in the table, we get: 0.77337. Since we are using a positive z table, we can use this value for the area left of the score by subtracting 1 - 0.77337: 0.22663 &rarr; About 22.663% watch less than 14 hours.  

The z-score for 24 is `r z24`. Looking up that value in the table, we get: 0.69146. Since we are using a positive z table, we can use this value for the area left of the score: Around 69.146% watch less than 24 hours. 
...
Look up the values in the z table to find the areas under the curve -- 0.22633 for 14, and 0.94738 (taking the lower bound of the range). To find the percentage between those two values, we subtract them.  
$$
P(-0.75 < Z < 1.625) = 0.94738 - 0.22633 = 0.72105
$$
So around 72.105% of people watch between 14 and 33 hours.  
...
The value at the middle of the sampling distribution should be: \$97,941.  

(b) Would this sampling distribution have a positive skew? Why or why not?  
No. [ADD EXPLANATION HERE]. This is because the sample statistics taken from the population are random...
...
95% of all observations would be between \$68,570.94 and \$83,417.06

```{r}
lower <- 1.96 * (76021/ sqrt(400))
upper <- -1.96 * (76021/ sqrt(400))
76021 - lower
76021 - upper
```
(e) What is the probability that your sample mean would be greater than \$101,742?  

Using for formula for the z-score:  
$$
Z = \frac{101742 - 97941}{76021} = 0.04999934
$$
Using the z table, the probability that the mean is greater than \$101,742 is $1 - .51994 \approx .480$ 
