...
\begin{aligned}
\text{Owns Home}: \\ 
Pr(y_i = 1|x_i) = \frac{\exp(x_i \beta_2)}{1 + \sum_{j=2}^{J} \exp(x_i \beta_j)} \\
Pr(y_i = 1|x_i) = \frac{1}{1 + .322 + .37 + .06} \\
Pr(y_i = 1|x_i) = [FILL IN] \\
\end{aligned}
$$
...
#### (g)  
Change the base category to "own home with no payments" and re-run the model. Examine the coefficients and compare them to the earlier estimation. How do you interpret the differences?   

```{r}
# relevel to make Pay Mortgage the first level
anes <- anes %>% mutate(HomeOwnership = forcats::fct_relevel(HomeOwnership, 'Own home with no payments due'))
...
According to the estimates, a person with these characteristics is predicted to have 1.53
memberships. This is slightly higher than the overall average number of memberships for the sample.  

#### (d)  
By hand, calculate the probability that this person belongs to 2 groups.  

```{r}
# using r to double check
dpois(2, prediction_1$estimate)
```
...
The negative coeﬀicient on `Voted2016` and `Health` indicates that people who voted in 2016 and that are in better health are less likely to be in the "always zero" category. Both coefficients are statistically significant.  
...
Using the zero-inflated model yields a prediction of 1.71, which is slightly higher than the poisson model.  
...
A person with these characteristics is expected to have 1.6 memberships. 
