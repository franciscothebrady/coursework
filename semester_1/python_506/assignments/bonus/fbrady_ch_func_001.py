...
def get_lowest_sugar_content(cereals, headers):
    ...
    for cereal in cereals:
        brand = get_cereal_attribute(
            cereal=cereal, headers=headers, header="brand"
        )  # TODO call function
        serving_size_gm = get_cereal_attribute(
            cereal=cereal, headers=headers, header="serving_size_gm"
        )  # TODO call function
        sugar_gm = get_cereal_attribute(
            cereal=cereal, headers=headers, header="sugar_gm"
        )  # TODO call function
        sugar_content = calculate_sugar_content(
            serving_size_gm=serving_size_gm, sugar_gm=sugar_gm, format_pct=False
        )  # TODO call function (return number not percent)
...
min_sugar_cereals = get_lowest_sugar_content(
    headers=data[0], cereals=data[1:]
)  # TODO: Call function (access cereals and headers in data)
