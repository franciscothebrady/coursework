...
# TODO 1.1
# create empty list for scifi tv shows
scifi_tv_shows = []

# TODO 1.2
# append scifi into list
# first split on second element (genre)
# search genre list
for show in tv_shows[1:]:
    # take second element in show as genre
    genre = show.split("|")[1]
    # print(genre)
    # search genre for scifi
    if "scifi" in genre.lower():
        # if true then append
...
for i in range(len(new_tv_shows)):
    streamer = new_tv_shows[i][5]
    if streamer.lower() == "netflix":
        netflix_new_tv_shows.append(new_tv_shows[i][0])
# print(netflix_new_tv_shows)
...
for show in tv_shows:
    # split on |
    show = show.split(" | ")
    # append each new list into list
    cleaned_tv_shows.append(show)
print(f"\n2.2: cleaned_tv_shows (first 5 elements) = {cleaned_tv_shows[:5]}")

# TODO 2.3
for show in range(len(cleaned_tv_shows)):
    # convert 4th and 5th value to float
    cleaned_tv_shows[show][3] = float(cleaned_tv_shows[show][3])
    cleaned_tv_shows[show][4] = float(cleaned_tv_shows[show][4])
print(f"\n2.3: cleaned_tv_shows (first 5 elements) = {cleaned_tv_shows[:5]}")
# TODO 2.4
for show in range(len(new_tv_shows)):
    # convert 4th and 5th values to float
    new_tv_shows[show][3] = float(new_tv_shows[show][3])
    new_tv_shows[show][4] = float(new_tv_shows[show][4])
print(f"\n2.4: new_tv_shows (first 5 elements) = {new_tv_shows[:5]}")

# extend cleaned tv shows with new tv shows
cleaned_tv_shows.extend(new_tv_shows)
# useful to print out each element on it's own line
# for line in cleaned_tv_shows_check:
#     print(line)
for line in cleaned_tv_shows:
    print(line)
...
for show in cleaned_tv_shows:
    # if genre == crime
    # remove whitespace
    if show[1].lower().strip() == "crime":
        temp = []
        # put title, year, and tomato rating into list
        temp.append(show[0])
        temp.append(show[2])
        temp.append(show[4])
        crime_genre.append(temp)
...
# TODO 4.2
for show in cleaned_tv_shows:
    print(f"\nrating for {show[0]}: {show[4]}")
    rating = show[4]
    total_rotten_tomato_ratings = total_rotten_tomato_ratings + rating
    print(f"\n total ratings: {total_rotten_tomato_ratings}")
...
# TODO 6.2
for show in cleaned_tv_shows:
    # if show is on netflix
    streamer = show[5].lower()
    if streamer == "netflix":
        select_content.append(show)
    else:
        non_select_content.append(show)
...
# TODO 8.2
for show in range(len(cleaned_tv_shows)):
    genre = cleaned_tv_shows[show][1].lower().strip()
